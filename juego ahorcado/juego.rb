...
class Juego
    ...
    def comenzar
        puts "Bienvenido al ahorcado, debes adivinar la palabra secreta, para comenzar te daré una pista"
        puts "La palabra secreta comienza con #{@palabra_secreta[0]}"  
        ...
        def he_ganado?
            if @vidas > 0
                puts "GANASTE! y aún te quedaban #{@vidas} intentos"
            else 
                puts "AHORCADO! X.X"
            end
